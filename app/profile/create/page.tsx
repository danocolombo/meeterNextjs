...
export default async function CreateProfilePage() {
    ...
    try {
        ...
        //* ---------------------------------
        const authRequest = {
            username: clerkCurrentUser?.privateMetadata?.meeter?.username,
            id: clerkCurrentUser?.privateMetadata?.meeter?.sub,
            email: clerkPrimaryEmailAddress.emailAddress,
        };
        // printObject('🥖🥖🥖APC:36--authRequest:\n', authRequest);
        try {
            ...
            userProfile?.affiliations?.forEach((affiliation: any) => {
                if (
                    affiliation.organization_id === userProfile.default_org_id
                ) {
